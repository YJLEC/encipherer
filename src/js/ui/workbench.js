...
(function (root, factory) {
  ...
  /* ================= 表头行 ================= */
  // 当前文件+sheet 的表头行(0 基);null 表示无表头(所有行都参与处理)
  function headerRowOf(fid, sheetIdx) {
    var en = st.files.filter(function (e) { return e.wb.id === fid; })[0];
    var sheet = en && en.wb.sheets[sheetIdx];
    if (!sheet) return null;
    var mode = st.headers[fid + ':' + sheetIdx];
    if (mode === undefined || mode === 'auto') {
      return util.detectHeaderRow(sheet.rows).row;
    }
    if (mode === 'none') return null;
    return mode;
  }
  function activeHeaderRow() {
    var en = activeFile();
    return en ? headerRowOf(en.wb.id, st.activeSheet) : null;
  }
  // 重建表头行下拉(文件/sheet 切换或数据变化时调用)
  function rebuildHeaderSel() {
    if (!st.els.headerSel) return;
    var en = activeFile();
    st.els.headerSel.textContent = '';
    if (!en) { st.els.headerSel.style.display = 'none'; return; }
    st.els.headerSel.style.display = '';
    var key = en.wb.id + ':' + st.activeSheet;
    var cur = st.headers[key] === undefined ? 'auto' : st.headers[key];
    var sheet = en.wb.sheets[st.activeSheet] || { rows: [] };
    var autoRow = util.detectHeaderRow(sheet.rows).row;
    var opts = [
      { v: 'auto', t: '表头:自动' + (autoRow !== null ? '(第' + (autoRow + 1) + '行)' : '(未识别)') },
      { v: 'none', t: '表头:无' }
    ];
    var maxRow = Math.min(10, sheet.rows.length);
    for (var i = 0; i < maxRow; i++) opts.push({ v: String(i), t: '表头:第' + (i + 1) + '行' });
    opts.forEach(function (o) {
      st.els.headerSel.appendChild(util.el('option', { value: o.v, text: o.t }));
    });
    st.els.headerSel.value = String(cur);
  }
  ...
  function doScan(sheet) {
    ...
    var rows = sheet.rows;
    var en = activeFile();
    var hr = en ? headerRowOf(en.wb.id, st.activeSheet) : null;
    for (var c = 0; c < sheet.maxCols; c++) {
      // ① 表头关键词命中 → 直接给类型建议(显示"表头"徽标)
      var headerText = hr != null ? excel.cellText((rows[hr] || [])[c]) : '';
    ...
    try {
      ...
      for (var f = 0; f < targets.length; f++) {
        ...
        for (var ri = 0; ri < rules.length; ri++) {
          ...
          var rows = sheet.rows;
          var t = rule.target;
          var hr = headerRowOf(wb.id, rule.sheet); // 表头行不参与脱敏
          var r1, r2, c1, c2;
          if (t.kind === 'col') { r1 = hr != null ? hr + 1 : 0; r2 = rows.length - 1; c1 = t.c; c2 = t.c; }
          ...
          for (var r = r1; r <= r2; r++) {
            if (r === hr) continue; // 表头行(范围规则跨到表头行时跳过)
            var row = rows[r] || [];
  ...
  function mount(container) {
    ...
    // 表头行选择:自动识别 / 无表头 / 指定行(表头行不参与脱敏)
    st.els.headerSel = util.el('select', { class: 'select', title: '表头行(该行不参与脱敏)' });
    st.els.headerSel.addEventListener('change', function () {
      var en = activeFile();
      if (!en) return;
      var v = st.els.headerSel.value;
      st.headers[en.wb.id + ':' + st.activeSheet] = v === 'auto' || v === 'none' ? v : Number(v);
      util.toast('表头行已更新,该行不再参与脱敏', 'info');
      refreshOverlays();
      refreshTip();
    });
    rebuildHeaderSel();
    ...
    var mid = util.el('section', { class: 'ec-wb-mid' }, [
      ...
      util.el('div', { class: 'ec-toolbar' }, [
        st.els.headerSel, st.els.piiSel, applyBtn, scanBtn, st.els.previewBtn, clearSelBtn,
